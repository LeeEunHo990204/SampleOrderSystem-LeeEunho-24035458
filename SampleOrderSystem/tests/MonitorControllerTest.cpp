...
TEST_F(MonitorControllerTest, 상태별_주문_집계_REJECTED_제외) {
    ...
    auto makeOrder = [](const std::string& id, const std::string& sid, OrderStatus st) {
        Order o(id, sid, "고객A", 10);
        o.setStatus(st);
        return o;
    };
    orders.push_back(makeOrder("ORD-001", "S-001", OrderStatus::RESERVED));
    orders.push_back(makeOrder("ORD-002", "S-001", OrderStatus::RESERVED));
    orders.push_back(makeOrder("ORD-003", "S-001", OrderStatus::CONFIRMED));
    orders.push_back(makeOrder("ORD-004", "S-001", OrderStatus::PRODUCING));
    orders.push_back(makeOrder("ORD-005", "S-001", OrderStatus::RELEASE));
    orders.push_back(makeOrder("ORD-006", "S-001", OrderStatus::REJECTED));
...
TEST_F(MonitorControllerTest, 재고_상태_SUFFICIENT_재고가_필요수량_이상) {
    std::vector<Sample> samples = {
        Sample("S-001", "실리콘 웨이퍼-8인치", 3.0, 0.85, 200)
    };
    sampleRepo_.saveAll(samples);

    std::vector<Order> orders;
    Order o("ORD-001", "S-001", "고객A", 100);
...
TEST_F(MonitorControllerTest, 재고_상태_SHORTAGE_재고가_필요수량_미만) {
    std::vector<Sample> samples = {
        Sample("S-001", "실리콘 웨이퍼-8인치", 3.0, 0.85, 30)
    };
    sampleRepo_.saveAll(samples);
...
TEST_F(MonitorControllerTest, 재고_상태_DEPLETED_재고가_0) {
    std::vector<Sample> samples = {
        Sample("S-001", "실리콘 웨이퍼-8인치", 3.0, 0.85, 0)
    };
    sampleRepo_.saveAll(samples);
...
TEST_F(MonitorControllerTest, 잔여율_계산_정확성) {
    std::vector<Sample> samples = {
        Sample("S-001", "웨이퍼A",  3.0, 0.85,  80),
        Sample("S-002", "웨이퍼B",  3.0, 0.85,   0),
        Sample("S-003", "웨이퍼C",  3.0, 0.85, 100)
    };
    sampleRepo_.saveAll(samples);

    std::vector<Order> orders;
    {
        Order o("ORD-001", "S-001", "고객A", 20);
        o.setStatus(OrderStatus::RESERVED);
        orders.push_back(o);
    }
    {
        Order o("ORD-002", "S-002", "고객B", 100);
        o.setStatus(OrderStatus::RESERVED);
        orders.push_back(o);
    }
